...
public class UjukomaGrading {
    ...
    @Test
    public void test05b_rec_types_grading() {
        test05_rec_types();
        legal("(5.05)");
        legal("(5.05) + (7.444)");
        legal("(x+y)+(y+x)");
    ...
    public void test07_ast_ops() {
        ...
        checkAstEquality("a-b", op('-', var('a'), var('b')));
    }
    ...
    public void test09_ast_prioriteet() {
        ...
        checkAstEquality("a - b / c", op('-', var('a'), op('/', var('b'), var('c'))));
    }
    ...
    @Test
    public void test10b_ast_all_grading() {
        test10_ast_all();
        checkAstEquality("x - y - z", op('-', op('-', var('x'), var('y')), var('z')));
        checkAstEquality("1-2+3", op('+', op('-', lit(1), lit(2)), lit(3)));
        checkAstEquality("1+2-3", op('-', op('+', lit(1), lit(2)), lit(3)));
        checkAstEquality("1.0-2.0+3.0", op('+', op('-', lit(1.0), lit(2.0)), lit(3.0)));
        checkAstEquality("1.0+2.0-3.0", op('-', op('+', lit(1.0), lit(2.0)), lit(3.0)));
        checkAstEquality("1.0*2.0/3.0", op('/', op('*', lit(1.0), lit(2.0)), lit(3.0)));
        checkAstEquality("1.0/2.0*3.0", op('*', op('/', lit(1.0), lit(2.0)), lit(3.0)));
    }
